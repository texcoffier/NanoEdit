				   ...
				   )
{
  ...
  if ( profondeur <= 0 )
	...
	{
	  glPushMatrix() ;
	  glRotatef(o->atd.table[i].y*180/M_PI, 0, 0, 1) ;
	  glRotatef(o->atd.table[i].x*180/M_PI, 1, 0, 0) ;
	  glScalef(o->atd.table[i].z, o->atd.table[i].z, o->atd.table[i].z) ;
	  affiche_arbre_recursif(profondeur, o, v) ;
	  glPopMatrix() ;
	}
      glPopMatrix() ;
    }
