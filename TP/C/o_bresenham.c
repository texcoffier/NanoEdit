...
/*EXCOIN*/
/************************************************************************/
/* Tracé de droite GENERIQUE           					*/
/************************************************************************/

#define DROITE(DX,DY,TEST,INCX,INCY)			\
	{ int c1, c2, c ;				\
	  c1 = 2*(DY) ;					\
          c  = c1 - (DX) ;				\
          c2 = c - (DX) ;				\
	  while( TEST )					\
               {					\
      		      ...
      		      glVertex2f(x1+1-0.5, y1  -0.5) ;	\
      		      glVertex2f(x1+1-0.5, y1+1-0.5) ;	\
      		      glVertex2f(x1  -0.5, y1+1-0.5) ;	\
		      if ( c<0 ) c += c1 ;		\
		        else { c += c2 ; INCY ; }	\
		      INCX ;				\
	       }					\
...
static void affiche_segment_droite(struct segment_droite *o)
{
   ...
   * La programmation est optimale en vitesse car on écrit
   * réellement les 8 programmes différents.
   */
  if (dx>0)
        { if (dy>0) {  if (dx>dy) DROITE( dx, dy,x1<=x2,x1++,y1++)  
                             else DROITE( dy, dx,y1<=y2,y1++,x1++) }
               else { if (dx>-dy) DROITE( dx,-dy,x1<=x2,x1++,y1--)
			     else DROITE(-dy, dx,y1>=y2,y1--,x1++) } }
  else
        { if (dy>0) { if (-dx>dy) DROITE(-dx, dy,x1>=x2,x1--,y1++) 
			     else DROITE( dy,-dx,y1<=y2,y1++,x1--) }
