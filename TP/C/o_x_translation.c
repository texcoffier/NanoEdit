...
static void affiche_translation(struct translation *o)
{
  ...
  /*EXCOIN*/
  {
    ...
    for( v=0; v<=o->nb_coupes ; v++ )
      {
	u = v/(float)o->nb_coupes ;
	h = u*(o->taille_fin - o->taille_debut)+o->taille_debut ;
	glBegin(GL_LINE_STRIP) ;
	for( i = 0             ; /* Premier point du contour */
	     i < o->contour.nb ; /* Pour tous les points du contour */
	     i++
	     )
	  glVertex3f(o->contour.table[i].x*h+u*o->vecteur.x,
		     o->contour.table[i].y*h+u*o->vecteur.y,
		     o->contour.table[i].z*h+u*o->vecteur.z) ;
	glEnd() ;
      }
	for( i = 0             ; /* Premier point du contour */
	     i < o->contour.nb ; /* Pour tous les points du contour */
 ...
 */
CLASSE(extrusion_translation, struct translation,
       Edite Extrait Sauve
